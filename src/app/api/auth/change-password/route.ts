...
export async function POST(req: Request) {
  ...
  if (token) {
    try {
      ...
      user = await prisma.cordinatorUser.findUnique({ where: { id: decoded.userId } });
    } catch {
      return NextResponse.json({ error: 'Invalid token.' }, { status: 401 });
    }
  } else if (userId) {
    user = await prisma.cordinatorUser.findUnique({ where: { id: userId } });
  ...
  await prisma.cordinatorUser.update({
    where: { id: user.id },

...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setError("");
    setSuccess("");
    const res = await fetch("/api/auth1/signup", {
      method: "POST",
    ...
    setTimeout(() => router.push("/login-ram"), 2000);
  };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-yellow-100 to-orange-200">
      <div className="w-full max-w-sm p-6 bg-white rounded-xl shadow-lg border border-yellow-300">
        <div className="flex flex-col items-center mb-6">
          ...
          <h2 className="text-xl font-bold text-yellow-700 mt-2">Sign Up for accesing course recording</h2>
        </div>
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <input type="email" className="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-yellow-400" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} />
